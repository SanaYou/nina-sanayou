"""Nina chatbot — Vercel Functions entrypoint (FastAPI)."""
...
# ── Paths (relatief aan project root, niet aan dit bestand) ──────────────────
...
def load_articles_index() -> list:
    """Laad alle artikelen als geindexeerde documenten voor RAG."""
...
def validate_articles(index: list):
    ...
    for article in index:
        if article["collection"] == "Algemeen":
            raw = KNOWLEDGE_DIR / "articles" / f"{article['id']}.md"
            if raw.exists():
...
Je helpt studenten en geinteresseerden met hun vragen. Je communiceert warm, professioneel en to-the-point, in de stem van Sandy Karsten, hoofddocent van SanaYou YOGAcademy. Houd je antwoorden kort en natuurlijk, nooit lange opsommingen van wat je allemaal kunt.
...
- Stel nooit meer dan een vervolgvraag per bericht
...
- "Wat is yin/yang/ashtanga yoga?" of vergelijkbare uitlegvragen: geef een korte uitleg, vraag dan: "Leuk dat je geinteresseerd bent! Gaat je voorkeur uit naar klassikaal of online?"
- "Is er een gratis proefles?" of vergelijkbare orienterende vragen: NIET antwoorden met opties, maar EERST vragen: "Gaat je interesse uit naar de klassikale lessen of een online opleiding?"
...
## Studenten die willen oefenen of feedback willen voor hun examen
...
- **Teaching Assessment Pack** (145 euro): lesvideo insturen + schriftelijk feedback + 60 min CoachCall met Sandy. Ideaal als examenvoorbereiding.
- **LIVE+ Support Pack** (75 euro/jaar): maandelijkse CoachCall, community, gastdocent Q&A's. Voor doorlopende vragen en structuur.
...
- "ik ben geexciteerd" -> "ik ben enthousiast"
...
def _taalcheck(client: anthropic.Anthropic, text: str) -> str:
    """Corrigeer Engelse calques in Nina's antwoord."""
    try:
...
async def get_articles():
    ...
    volgorde_path = KNOWLEDGE_DIR / "volgorde.json"
    volgorde = {}
...
def _send_escalation(name: str, email: str, summary: str, chat_messages: list):
    token = _hs_get_token()
    if not token:
        logger.error("Help Scout credentials niet geconfigureerd")
        return
    ...
    resp = _requests.post(
        ...
        json={
            ...
            "customer": {"email": email, "firstName": name},
